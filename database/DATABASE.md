...
D1へ保存する業務日時は、**日本時間（JST / UTC+09:00）のISO 8601形式TEXT**に統一する。
...
- `Z`（UTC）のまま業務日時を保存しない
- タイムゾーンなしの `YYYY-MM-DD HH:MM:SS` 形式を保存しない
- Workers側で現在日時を生成する場合は共通関数 `nowIso()` を使用する
- `nowIso()` は必ず `+09:00` 付きISO 8601を返す
- DDLで初期データ日時を生成する場合もJST `+09:00` 付きISO 8601を使用する
- 表示側でUTC→JST変換することを前提にせず、D1内の業務日時自体をJST表記へ統一する

日時の大小比較（例: START_DATE / END_DATE と現在時刻の available 判定）は、
ISO 8601 文字列の辞書順比較ではなく、日時（Date）へ変換して比較する。
保存日時はミリ秒付き（`...T06:30:00.000+09:00`）とミリ秒なし
（`9999-12-31T23:59:59+09:00`）が混在し得るため、辞書順比較では秒直後の
`.`（0x2E）と `+`（0x2B）の並びにより同一時刻付近で誤判定する可能性がある。
オフセットは全て `+09:00` に統一されているため、Date 変換に曖昧さはない。
...
一度LocalまたはProductionへ適用したmigrationファイルは変更しない。

仕様修正が必要な場合は、新しいmigrationを追加する。
...
設計書側の `database/DDL.sql` は「現在の正しい最終スキーマ・初期データ仕様」を示す正本として更新してよい。

そのため、適用済みmigrationの内容と最新 `DDL.sql` が完全一致しない場合がある。

新規環境はmigrationを番号順に適用し、既存環境と同じ最終状態へ到達させる。
